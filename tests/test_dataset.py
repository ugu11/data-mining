...
class TestDataset(unittest.TestCase):
    ...
    def test_readDataset(self):
        ...
        expected_feature_names = ['Nome', 'Laboratórios', ' Alg. Avançados']
        expected_numerical_cols = [1, 2, 3]
        expected_categorical_cols = [0] 
        expected_categories = {0: np.array(['Carina', 'Carla', 'Joaquim', 'José', 'João', 'Pedro', 'Ricardo',
       'Sónia', 'Telma', 'Vanda'], dtype='<U7')}
        ...
        result = dt.label == expected_label and \
                  ...
                  np.allclose(dt.y, expected_y, equal_nan=True)
    ...
    def test_replace_missing_values_dataset(self):
        """
        Test if the replace_missing_values_dataset function is correctly replacing all the missing values 
        in the dataset.
        """
        dt = Dataset('./datasets/notas.csv')
